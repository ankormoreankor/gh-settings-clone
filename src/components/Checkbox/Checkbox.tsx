...
export const Checkbox = ({
  ...
  const renderLabel = () => (
    <span className={clsx(scss.labelText, scss[size], isWithLeftLabel && scss.alignRight)} onClick={onLabelClick}>
      {labelText}
    </span>
  );
  ...
  return (
    <div className={clsx(scss.root, !!labelText && scss.labeled, disabled && scss.isDisabled, className)}>
      {!!labelText && isWithLeftLabel && renderLabel()}

      <RadixCheckbox.Root
        data-testid={testId}
        className={clsx(scss.checkbox, isRound && scss.isRound, disabled && scss.isDisabled, scss[size])}
