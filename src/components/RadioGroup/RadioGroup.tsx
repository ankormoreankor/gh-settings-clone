...
export const RadioButton = ({ size = 'medium', isDisabled, label, id, ...props }: RadioButtonProps) => {
  const RadioButtonComponent = () => (
    <RadixRadioGroup.Item
      {...props}
      id={id ?? props.value}
      className={clsx(scss.radioButton, utility.buttonReset, scss[size], isDisabled && scss.isDisabled)}
      disabled={isDisabled}
      data-element="radio-button"
    >
      <RadixRadioGroup.Indicator
        className={clsx(scss.radioButtonIndicator, scss[size], isDisabled && scss.isDisabled)}
  ...
  return (
    <>
      {!!label && (
        <label className={clsx(scss.label, scss[size])} data-element="radio-button-label">
  ...
  ({ className, items, isDisabled: isRadioGroupDisabled, size, ...restProps }, ref) => {
    ...
    return (
      <RadixRadioGroup.Root
        {...restProps}
        ref={ref}
        className={clsx(scss.radioGroupRoot, className)}
