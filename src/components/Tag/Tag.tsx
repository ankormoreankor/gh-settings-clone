...
export const Tag = ({
  ...
  return (
    <Tooltip
      ...
      className={clsx(scss.tooltip, utility.ellipsis)}
    ...
    >
      ...
      >
        ...
        {!!children && (
          <span className={clsx(scss.text, utility.ellipsis, isDeleted && scss.isDeleted)}>{children}</span>
        )}

        {isWithButtons && (
          <div className={clsx(scss.buttons)}>
            {isEditable && (
              <Button
                variant="transparent"
                size="tiny"
