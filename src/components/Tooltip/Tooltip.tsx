    ...
    <RadixTooltip.Provider delayDuration={delayDuration}>
      <RadixTooltip.Root {...controlProps} defaultOpen={isDefaultOpen}>
        <RadixTooltip.Trigger asChild>
          <span {...triggerProps} data-element="tooltip-trigger" className={clsx(scss.tooltipRoot, className)}>
            {children}
          </span>
        ...
        {!isDisabled && (
          <RadixTooltip.Portal {...portalProps}>
            <RadixTooltip.Content
              ...
              style={{ zIndex: 100000 }}
              className={clsx(scss.content, contentProps?.className)}
              side={placement ?? contentProps?.side}
            >
              <span className={scss.tooltipContentContainer}>{content}</span>

              {isHasArrow && (
                <RadixTooltip.Arrow className={clsx(scss.tooltipArrow, arrowClassName)} width={11} height={5} />
              )}
            </RadixTooltip.Content>
          </RadixTooltip.Portal>
        )}
